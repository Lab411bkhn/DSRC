...
ssize_t
__LIBFAST__ber_fetch_length(int _is_constructed, const void *bufptr, size_t size,
		ber_tlv_len_t *len_r) {
...
ssize_t
__LIBFAST__ber_skip_length(asn_codec_ctx_t *opt_codec_ctx,
	 ...
	 */
	ll = __LIBFAST__ber_fetch_length(_is_constructed, ptr, size, &vlen);
	if(ll <= 0) return ll;
	 ...
	 * Indefinite length!
	 */
	__LIBFAST__ASN_DEBUG("Skipping indefinite length");
	for(skip = ll, ptr = ((const char *)ptr) + ll, size -= ll;;) {
		ber_tlv_tag_t tag;

		/* Fetch the tag */
		tl = __LIBFAST__ber_fetch_tag(ptr, size, &tag);
		if(tl <= 0) return tl;
...
size_t
__LIBFAST__der_tlv_length_serialize(ber_tlv_len_t len, void *bufp, size_t size) {
