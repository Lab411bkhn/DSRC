...
static ssize_t __LIBFAST__der_write_TL(ber_tlv_tag_t tag, ber_tlv_len_t len,
 ...
 */
asn_enc_rval_t
__LIBFAST__der_encode(asn_TYPE_descriptor_t *type_descriptor, void *struct_ptr,
	asn_app_consume_bytes_f *consume_bytes, void *app_key) {

	__LIBFAST__ASN_DEBUG("DER encoder invoked for %s",
...
/*
 * Argument type and callback necessary for __LIBFAST__der_encode_to_buffer().
 */
...
/*
 * A variant of the __LIBFAST__der_encode() which encodes the data into the provided buffer
 */
asn_enc_rval_t
__LIBFAST__der_encode_to_buffer(asn_TYPE_descriptor_t *type_descriptor, void *struct_ptr,
	...
	ec = type_descriptor->der_encoder(type_descriptor,
		struct_ptr,	/* Pointer to the destination structure */
		0, 0, __LIBFAST__encode_to_buffer_cb, &arg);
	if(ec.encoded != -1) {
 ...
 */
ssize_t
__LIBFAST__der_write_tags(asn_TYPE_descriptor_t *sd,
	...
	__LIBFAST__ASN_DEBUG("Writing tags (%s, tm=%d, tc=%d, tag=%s, mtc=%d)",
		sd->name, tag_mode, sd->tags_count,
		__LIBFAST__ber_tlv_tag_string(tag),
		tag_mode
			?(sd->tags_count+1
	...
	if(tag_mode) {
		/*
		 * Instead of doing shaman dance like we do in __LIBFAST__ber_check_tags(),
	...
	for(i = tags_count - 1; i >= 0; --i) {
		lens[i] = __LIBFAST__der_write_TL(tags[i], overall_length, 0, 0, 0);
	...
	__LIBFAST__ASN_DEBUG("%s %s TL sequence (%d elements)",
	...
	for(i = 0; i < tags_count; i++) {
		...
		len = __LIBFAST__der_write_TL(tags[i], lens[i], cb, app_key, _constr);
		if(len == -1) return -1;
	}
...
static ssize_t
__LIBFAST__der_write_TL(ber_tlv_tag_t tag, ber_tlv_len_t len,
	...
	tmp = __LIBFAST__ber_tlv_tag_serialize(tag, buf, buf_size);
	if(tmp == -1 || tmp > (ssize_t)sizeof(buf)) return -1;
	size += tmp;

	/* Serialize length (L from TLV) into possibly zero-length buffer */
	tmp = __LIBFAST__der_tlv_length_serialize(len, buf+size, buf_size?buf_size-size:0);
	if(tmp == -1) return -1;
	size += tmp;
