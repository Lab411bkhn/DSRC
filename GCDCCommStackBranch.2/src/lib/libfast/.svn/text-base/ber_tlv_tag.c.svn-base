...
ssize_t
__LIBFAST__ber_fetch_tag(const void *ptr, size_t size, ber_tlv_tag_t *tag_r) {
	ber_tlv_tag_t val;
...
ssize_t
__LIBFAST__ber_tlv_tag_fwrite(ber_tlv_tag_t tag, FILE *f) {
	char buf[sizeof("[APPLICATION ]") + 32];
	ssize_t ret;

	ret = __LIBFAST__ber_tlv_tag_snprint(tag, buf, sizeof(buf));
...
ssize_t
__LIBFAST__ber_tlv_tag_snprint(ber_tlv_tag_t tag, char *buf, size_t size) {
	char *type = 0;
	int ret;
...
char *
__LIBFAST__ber_tlv_tag_string(ber_tlv_tag_t tag) {
	static char buf[sizeof("[APPLICATION ]") + 32];

	(void)__LIBFAST__ber_tlv_tag_snprint(tag, buf, sizeof(buf));
...
size_t
__LIBFAST__ber_tlv_tag_serialize(ber_tlv_tag_t tag, void *bufp, size_t size) {
