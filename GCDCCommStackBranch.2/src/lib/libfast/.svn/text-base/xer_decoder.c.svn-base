 ...
 */
asn_dec_rval_t
__LIBFAST__xer_decode(asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
...
static int
__LIBFAST__xer__token_cb(pxml_chunk_type_e type, const void *_chunk_data, size_t _chunk_size, void *key) {
 ...
 */
ssize_t
__LIBFAST__xer_next_token(int *stateContext, const void *buffer, size_t size, pxer_chunk_type_e *ch_type) {
	struct xer__cb_arg arg;
	int new_stateContext = *stateContext;
	ssize_t ret;

	arg.callback_not_invoked = 1;
	ret = __LIBFAST__pxml_parse(&new_stateContext, buffer, size, __LIBFAST__xer__token_cb, &arg);
	if(ret < 0) return -1;
...
xer_check_tag_e
__LIBFAST__xer_check_tag(const void *buf_ptr, int size, const char *need_tag) {
	...
	if(size < 2 || buf[0] != LANGLE || buf[size-1] != RANGLE) {
		if(size >= 2)
		__LIBFAST__ASN_DEBUG("Broken XML tag: \"%c...%c\"", buf[0], buf[size - 1]);
		return XCT_BROKEN;
	}
...
#define	RETURN(_code)	do {					\
		...
		if(rval.code != RC_OK)				\
			__LIBFAST__ASN_DEBUG("Failed with %d", rval.code);	\
		return rval;					\
	} while(0)
 ...
 */
asn_dec_rval_t
__LIBFAST__xer_decode_general(asn_codec_ctx_t *opt_codec_ctx,
	...
	for(;;) {
		...
		__LIBFAST__ASN_DEBUG("Unexpected XML tag (expected \"%s\")", xml_tag);
...
int
__LIBFAST__xer_is_whitespace(const void *chunk_buf, size_t chunk_size) {
 ...
 */
int
__LIBFAST__xer_skip_unknown(xer_check_tag_e tcv, ber_tlv_len_t *depth) {
	assert(*depth > 0);
	switch(tcv) {
	case XCT_BOTH:
	case XCT_UNKNOWN_BO:
