...
int __LIBFAST__get_asn1c_environment_version(void);	/* Run-time version */
 ...
 */
#ifndef	__LIBFAST__ASN_DEBUG	/* If debugging code is not defined elsewhere... */
...
#define	__LIBFAST__ASN_DEBUG(fmt, args...)	do {			\
...
#define	__LIBFAST__ASN_DEBUG	__LIBFAST__ASN_DEBUG_f
#endif	/* __GNUC__ */
#else	/* EMIT_ASN_DEBUG != 1 */
static inline void __LIBFAST__ASN_DEBUG(const char *fmt, ...) { (void)fmt; }
#endif	/* EMIT_ASN_DEBUG */
#endif	/* __LIBFAST__ASN_DEBUG */
...
static inline int
__LIBFAST___ASN_STACK_OVERFLOW_CHECK(asn_codec_ctx_t *ctx) {
	if(ctx && ctx->max_stack_size) {
		...
		if(usedstack < -(ptrdiff_t)ctx->max_stack_size) {
			__LIBFAST__ASN_DEBUG("Stack limit %ld reached",
